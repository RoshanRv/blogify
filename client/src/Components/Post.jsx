...
const Post = () => {
    ...
    const handleEditSave = ()=>{
        if(data.title===''|data.post===''){
            ...
            if(isEditing){
                setIsEditing(false)
                Axios.post(`https://blogify--react.herokuapp.com/update/${id}`,{title:data.title,post:data.post}).then(
                    setShowMsg(true),
                        setTimeout(()=>{
                            setShowMsg(false)
                        },2000)
    ...
    const handleDelete = ()=>{
        Axios.get(`https://blogify--react.herokuapp.com/posts/delete/${id}`).then(
            setTimeout(()=>{
                navigate(`/${loginID}`)
            },500)
        )
